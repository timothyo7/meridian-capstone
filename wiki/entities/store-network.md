# Store network

Meridian Markets' fourteen stores, per the [client brief](../sources/client-brief.md).

## What we know

- Fourteen stores across Los Angeles, Orange, and Ventura counties
  ([brief](../sources/client-brief.md)).
- Grew from six to fourteen stores in five years
  ([brief](../sources/client-brief.md)).
- The eight newer stores came mostly from leases taken over from chains that
  exited neighborhoods Meridian judged underserved
  ([brief](../sources/client-brief.md)).
- Roughly $78M annual revenue and about 620 employees across the chain
  ([brief](../sources/client-brief.md)).
- Store attributes available in the extract: square footage, opening date,
  and lease terms ([brief](../sources/client-brief.md)).
- Performance is uneven — some stores "took off immediately," others were
  "slower to find their footing" ([brief](../sources/client-brief.md)).

## Open questions

Tracked in the [register](../analysis/open-questions.md) as Q15, Q16, Q17.

## Related pages

- [Store performance variation](../concepts/store-performance-variation.md)
- [Pasadena site](pasadena-site.md)
